...
[BCs]
  ...
  [./flux_phi2]
    ...
    I = 0.0372588
  [../]
  # Bai et al. (J. Power Sources 422, 2019) Eq. (10): -lambda dphi1/dX = 0 at
  # X = 0, i.e. the anode-side face of phis is zero-FLUX, not pinned. The
  # original macro_original.i pinned phis = 0 there instead, which contradicts
  # Eq. (10) and puts an electron sink at the far side of the separator.
  # Dropping it leaves phis pure-Neumann: the current injected at cat_cc has
  # nowhere to go but the reaction. phie carries the reference instead, per
  # the group's FE2 convention (phie = 0 at the current collector).
  # Everything else in this file is Armin's original, untouched.
  [./PhiE_ref]
    type = DirichletBC
    variable = phie
    value = 0.0
